...
config.plugins.MetrixWeather.woeid = ConfigNumber(default="2344117") #Location (visit metrixhd.info)
...
class MetrixWeatherUpdater(Renderer, VariableText):
	...
	def GetWeather(self):
		...
		# where location in (select id from weather.search where query="izmir, turkey")
